...
class HomeController extends BaseController<void> {
  ...
  Future<void> _getCurrentLocation() async {
    try {
      ...
      mapController.move(currentPosition.value!, 16);
      showSuccess('Thành công', 'Cập nhật vị trí hiện tại');
    } catch (e, stack) {
  ...
  Future getData() async {
    // TODO: gọi API lấy danh sách spot (nếu có)
  }

  Future<void> showAddSpotBottomSheet() async {
    // Gọi bottom sheet thêm địa điểm
    final newSpot = await AddSpotBottomSheet.show(Get.context!);

    if (newSpot != null) {
      spots.add(newSpot);
      Get.snackbar('Thành công', 'Đã thêm địa điểm mới!',
...
//   if (newSpot != null) {
//     spots.add(newSpot);
//     if (newSpot.lat != 0 && newSpot.lng != 0) {
